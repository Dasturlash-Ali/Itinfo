...
exports.adminValidation = (data) => {
  const adminSchema = Joi.object({
    admin_name: Joi.string().trim().min(3).max(50).required().messages({
    ...
    admin_is_active: Joi.boolean().default(true).messages({
      "boolean.base": "Admin active status must be true or false",
    }),

    admin_is_creator: Joi.boolean().default(false).messages({
